...
export default class ComponentPacker {
     ...
     */
    async build(dir, packname, { kind, multi = false, identity, output, skip, test } = {}) {
        if (!dir) throw ErrNotADirectory('<none>');
        ...
        const packagefile = output ?? path.basename(dir);
        const archiveprops = await this.makeArchive(packagefile, found, components, identity, multi);
    ...
    async makeArchive(tharfile, files, components, identityfile, multi) {
        ...
        files.forEach(({ fslocation, modulelocation }) => {
            const name = multi ? modulelocation : path.join('/', components[0], modulelocation);
